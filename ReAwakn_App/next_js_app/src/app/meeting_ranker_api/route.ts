...
export async function POST(req: Request) {
  ...
  if (!slots?.length) return NextResponse.json({ top_slots: [], all_count: 0 });
  ...
  if (existingMeetings) {
    ...
    if (user1Meetings.length > 0) {
      user1.existingMeetings = user1Meetings.map((meeting: any) => ({
    ...
    if (user2Meetings.length > 0) {
      user2.existingMeetings = user2Meetings.map((meeting: any) => ({
        startUTC: meeting.start_time,
        endUTC: new Date(
          new Date(meeting.start_time).getTime() + 60 * 60 * 1000
        ).toISOString(),
